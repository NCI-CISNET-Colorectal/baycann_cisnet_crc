...
#BayCANN_version  <- "CRCSPIN_v1_20221011_1345"  #Version for poster SMDM and paper
...
outputs_file     <- "data-raw/20220325_CRC_SPIN/SimulatedTargets_20220325_1233CDT.csv"
selected_targets <- c(4:(length(data_sim_target)))

params_file      <- "data-raw/20220325_CRC_SPIN/SimulatedParameters_20220325_1233CDT.csv"
data_sim_param <- subset(data_sim_param, select = -c(iter,draw,step,seed,ar.birthcohort,
                                                     rf.proximal.shift, rf.adeno.incidence,
                                                     rf.adeno.growth, rf.preclin.trans, 
                                                     rf.sojourn.time, rf.oc.survival,sample_wt))
selected_params  <- colnames(data_sim_param)
...
data_sim_target <- data_sim_target[!v_NA,]    # keep only complete data
...
data_sim_param <- data_sim_param[!v_NA,]
...
data_true_targets  <- read_excel(targets_file)
...
mdl_string <- paste("model %>% layer_dense(units = n_hidden_nodes,  activation = activation_fun, input_shape = n_inputs) %>%",
...
param_poster$name <- str_replace(param_poster$name,"Corley_adeno_prev_fem_50_54","Prev 52 years")
param_poster$name <- str_replace(param_poster$name,"Corley_adeno_prev_fem_60_64","Prev 62 years")
param_poster$name <- str_replace(param_poster$name,"Corley_adeno_prev_fem_70_74","Prev 72 years")
param_poster$name <- str_replace(param_poster$name,"Corley_adeno_prev_fem_75plus","Prev 75+ years")

plot_CRCSPIN_ANN_prediction <- ggplot(data = param_poster, aes(x = model, y = pred)) +
  geom_point(alpha = 0.5, color = "tomato") +
...
if (Normalize_inputs) {
  file_perceptron <- "post_multi_perceptron_sigmoid_normal.stan"
} else {
...
df_samp_post_ann   <- melt(cbind(Distribution = "Posterior BayCANN CRC-SPIN",
